...
class simulation_2D(simulation):
    ...
    def Sim_step_graph(self,u, Δt, Δx, Δy,fronteiraX, fronteiraY,Force):
        ...
        duY[...,1,:],duY[...,2,:]=duY[...,2,:],duY[...,1,:]

        u1 = u - Δt*(duX+duY-Force(u))

        u1X=self.API.einsum('...ijk -> ...kij',u1)
        u1Y=self.API.einsum('...ijk -> ...jik',u1)
        u1Y[...,1,:],u1Y[...,2,:]=u1Y[...,2,:],u1Y[...,1,:]
        ...
        du1Y[...,1,:],du1Y[...,2,:]=du1Y[...,2,:],du1Y[...,1,:]

        u2 = (3*u + u1 - Δt*(du1X+du1Y-Force(u1))) / 4.0

        u2X=self.API.einsum('...ijk -> ...kij',u2)
        u2Y=self.API.einsum('...ijk -> ...jik',u2)
        u2Y[...,1,:],u2Y[...,2,:]=u2Y[...,2,:],u2Y[...,1,:]
