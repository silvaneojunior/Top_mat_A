...
def Burgers(u, t_final, Δx, CFL, fronteira, network=Null_net.Network):
    ...
    while tf.math.reduce_any(t < t_final):
        ...
        Δt = Δx*CFL/Λ
        u,index=Burgers_step(u,Δx,Δt,t,t_final, fronteira, network)
...
def Burgers_step(u,Δx,Δt,t,t_final, fronteira, network):
...
#     f_minus = (U_full - M*U_full)/2 # Fluxo negativo
    ...
    ω1=tf.broadcast_to(ω0,[tf.shape(f_minus)[0],tf.shape(f_minus)[1]-tf.shape(index_desc)[0]+1,3])
    
    ω0_Z_plus   = tf.expand_dims(WenoZ5ReconstructionMinus_weigth(f_plus_desc[:,:,:-1], beta_weight_full[:,:-1,:])[0],axis=0)
    ω0_Z_minus  = tf.expand_dims(WenoZ5ReconstructionPlus_weigth( f_minus_desc[:,:,1:], beta_weight_full[:,1:,:])[0],axis=0)
    
    ω0_Z_plus   = tf.concat([ω1,ω0_Z_plus],axis=1)
    ω0_Z_minus  = tf.concat([ω1,ω0_Z_minus],axis=1)
    
    index = tf.concat([index_cont, index_desc], axis = 0)
    index = tf.argsort(index, axis = 0)
    
    ω0_Z_plus = tf.gather(ω0_Z_plus, index, axis = 1)
    ω0_Z_minus  = tf.gather(ω0_Z_minus , index, axis = 1)
    
    tf.print(ω0_Z_plus.shape)
    tf.print((f_plus[:,:,:-1] @ C).shape)
    
    f_half_minus = tf.transpose(tf.math.reduce_sum(ω0_Z_plus * (f_plus[:,:,:-1] @ C)                      , axis=2, keepdims=True))
    f_half_plus  = tf.transpose(tf.math.reduce_sum(ω0_Z_minus * (tf.reverse(f_minus[:,:,1:], axis=[2]) @ C), axis=2, keepdims=True))
...
α0 = tf.constant(0.0, dtype = float_pres)
...
def WenoZ5ReconstructionMinus_weigth(u0, beta_weight):
    """
...
def WenoZ5ReconstructionPlus_weigth(u0, beta_weight):
    ω = WenoZ5ReconstructionMinus_weigth(tf.reverse(u0,axis=[2]), tf.reverse(beta_weight,axis=[2]))
    return ω

def WenoZ5ReconstructionMinus(u0, beta_weight):
    ω = WenoZ5ReconstructionMinus_weigth(u0, beta_weight)
