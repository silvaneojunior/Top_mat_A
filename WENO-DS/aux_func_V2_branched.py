...
def Edge_Detection(ω_minus, ω_plus):
    
    index_minus = tf.math.reduce_sum(a0 * tf.math.log(ω_minus) + b0, axis = [2]) > 0
    index_plus  = tf.math.reduce_sum(a0 * tf.math.log(ω_plus)  + b0, axis = [2]) > 0
...
def Graph_Burgers2(u, t_final, Δx, CFL, fronteira, network=Null_net.Network):
    ...
    while tf.math.reduce_any(t < t_final):
        ...
        Fhat, ω_minus, ω_plus   = DerivadaEspacial(u, Δx, fronteira, network)
        index_minus, index_plus = Edge_Detection(ω_minus, ω_plus)
        index = tf.math.logical_and(index_minus, index_plus)
        
        # SSP Runge-Kutta 3,3
        u1 = u - Δt*Fhat
        u2 = (3*u +   u1 -   Δt*FastDerivadaEspacial(u1, Δx, fronteira, index, network)) / 4.0
        u  = (  u + 2*u2 - 2*Δt*FastDerivadaEspacial(u2, Δx, fronteira, index, network)) / 3.0
        
        t  = t + Δt # Avançando no tempo
        
        #index = tf.math.logical_and(index_minus, index_plus)
        index = tf.logical_and(index[:,1:], index[:,:-1])
...
def FastDerivadaEspacial(U, Δx, AdicionaGhostPoints, index, network):
    """
    ...
    # 6 pontos e depois concatenar em um único tensor (com tf.concat) na primeira
    # dimensão
    U_full = tf.concat(
        tf.map_fn(
...
#     f_minus = (U_full - M*U_full)/2 # Fluxo negativo
    ...
    # Versão um pouco mais rápida pois faz separação única dos índices
    #------------------------------------------------------------------------------------------------------------------------------------
    ...
    f_half_minus_cont = tf.transpose(tf.math.reduce_sum(ω0 * (f_plus_cont[:,:,:-1] @ C)                      , axis=2, keepdims=True))
    f_half_plus_cont  = tf.transpose(tf.math.reduce_sum(ω0 * (tf.reverse(f_minus_cont[:,:,1:], axis=[2]) @ C), axis=2, keepdims=True))
    
    f_half_minus_desc = WenoZ5ReconstructionMinus(f_plus_desc[:,:,:-1], beta_weight_full[:,:-1,:])[0]
    f_half_plus_desc  = WenoZ5ReconstructionPlus( f_minus_desc[:,:,1:], beta_weight_full[:,1:,:])[0]
    
    index = tf.concat([index_cont, index_desc], axis = 0)
    index = tf.argsort(index, axis = 0)
    
    f_half_minus = tf.concat([f_half_minus_cont, f_half_minus_desc], axis = 1)
    f_half_plus  = tf.concat([f_half_plus_cont , f_half_plus_desc] , axis = 1)
    
    f_half_minus = tf.gather(f_half_minus, index, axis = 1)
    f_half_plus  = tf.gather(f_half_plus , index, axis = 1)
    #-------------------------------------------------------------------------------------------------------------------------------------
    
    # Versão um pouco mais lenta pois faz a separação dos índices para o fluxo positivo e negativo
    #------------------------------------------------------------------------------------------------------------------------------------
#     index_minus_cont = tf.where(index[0])[:,1]
#     index_minus_desc = tf.where(tf.math.logical_not(index[0]))[:,1]
#     index_plus_cont  = tf.where(index[1])[:,1]
#     index_plus_desc  = tf.where(tf.math.logical_not(index[1]))[:,1]
    
#     f_minus_cont = tf.gather(f_minus, index_plus_cont , axis = 1)
#     f_minus_desc = tf.gather(f_minus, index_plus_desc , axis = 1)
#     f_plus_cont  = tf.gather(f_plus , index_minus_cont, axis = 1)
#     f_plus_desc  = tf.gather(f_plus , index_minus_desc, axis = 1)
...
#     index_minus = tf.concat([index_minus_cont, index_minus_desc], axis = 0)
#     index_plus  = tf.concat([index_plus_cont , index_plus_desc ], axis = 0)
    
#     index_minus = tf.argsort(index_minus, axis = 0)
#     index_plus  = tf.argsort(index_plus , axis = 0)
    
#     f_half_minus = tf.concat([f_half_minus_cont, f_half_minus_desc], axis = 1)
#     f_half_plus  = tf.concat([f_half_plus_cont , f_half_plus_desc] , axis = 1)
    
#     f_half_minus = tf.gather(f_half_minus, index_minus, axis = 1)
#     f_half_plus  = tf.gather(f_half_plus , index_plus , axis = 1)
    #-------------------------------------------------------------------------------------------------------------------------------------
...
#     f_minus = (U_full - M*U_full)/2 # Fluxo negativo
    ...
    # Aplicar WENO em cada variável característica separadamente para depois juntar
    f_half_minus, ω_minus = WenoZ5ReconstructionMinus(f_plus[:,:,:-1], beta_weight_full[:,:-1,:])
    f_half_plus , ω_plus  = WenoZ5ReconstructionPlus( f_minus[:,:,1:], beta_weight_full[:,1:,:])
    Fhat         = (f_half_minus + f_half_plus)
    
    # Calculando uma estimava da derivada a partir de diferenças finitas
    Fhat = tf.transpose((Fhat[:,1:] - Fhat[:,:-1]) / Δx)
    
    return Fhat, ω_minus, ω_plus
...
ω0 = tf.constant([[[1/10, 6/10, 3/10]]], dtype = float_pres)
α0 = tf.constant(10.0, dtype = float_pres)

b0  = tf.math.lgamma(α0)
b0 -= tf.math.reduce_sum(tf.math.lgamma(α0*ω0))
b0 -= tf.math.lgamma(tf.constant(3, float_pres))
a0  = α0*ω0-1
...
def WenoZ5ReconstructionPlus(u0, beta_weight):
    ...
    fhat, ω = WenoZ5ReconstructionMinus(tf.reverse(u0,axis=[2]), tf.reverse(beta_weight,axis=[2]))
